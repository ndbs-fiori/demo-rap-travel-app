...
CLASS lhc_booking IMPLEMENTATION.
  ...
  METHOD calculateBookingID.
    ...
    DATA update TYPE TABLE FOR UPDATE ZI_RAP_Travel_np\\Booking.

    " Read all travels for the requested bookings.
    " If multiple bookings of the same travel are requested, the travel is returned only once.
    READ ENTITIES OF ZI_RAP_Travel_np IN LOCAL MODE
    ENTITY Booking BY \_Travel FIELDS ( TravelUUID )
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      READ ENTITIES OF ZI_RAP_Travel_np IN LOCAL MODE
      ENTITY Travel BY \_Booking FIELDS ( BookingID )
    ...
    ENTITY Booking UPDATE FIELDS ( BookingID ) WITH update
  ...
  METHOD calculateTotalPrice.
    ...
    READ ENTITIES OF ZI_RAP_Travel_np IN LOCAL MODE
    ENTITY Booking BY \_Travel FIELDS ( TravelUUID )
    WITH CORRESPONDING #( keys ) RESULT DATA(travels) FAILED DATA(read_failed).

    " Trigger calculation of the total price
    MODIFY ENTITIES OF ZI_RAP_Travel_np IN LOCAL MODE
    ENTITY Travel EXECUTE recalcTotalPrice
    FROM CORRESPONDING #( travels ) REPORTED DATA(execute_reported).
